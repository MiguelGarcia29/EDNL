...
inline void Abin<T>::insertarHijoIzqdo(Abin<T>::nodo n, const T &e)
{
    ...
    // AÒadir el nuevo nodo al final de la secuencia.
...
void Abin<T>::eliminarHijoIzqdo(Abin<T>::nodo n)
{
    ...
    assert(nodos[hizqdo].hizq == NODO_NULO && // Hijo izqdo.
           nodos[hizqdo].hder == NODO_NULO);  // n es hoja.
    if (hizqdo != numNodos - 1)
    {
        // Mover el ˙ltimo nodo a la posiciÛn del hijo izqdo.
        nodos[hizqdo] = nodos[numNodos - 1];
        // Actualizar la posiciÛn del hijo (izquierdo o derecho)
        ...
        // actualizar la posiciÛn del padre.
...
void Abin<T>::eliminarHijoDrcho(Abin<T>::nodo n)
{
    ...
    {
        // Mover el ˙ltimo nodo a la posiciÛn del hijo drcho.
